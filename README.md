# Депривация сна и производительность труда: сравнение методов причинного вывода

Учебный проект по машинному обучению и причинному выводу. Вопрос простой: если человек сам решает, высыпаться ему или нет, а не экспериментатор случайным образом лишает его сна, можно ли вообще корректно оценить, насколько недосып бьёт по производительности? Pilcher и Huffcutt ещё в 1996 году по итогам 19 экспериментов показали, что депривация сна ухудшает работоспособность практически по всем направлениям, а Gibson и Shrader в 2018-м на реальных данных о времени заката оценили, что дополнительный час сна поднимает заработок на 1–5%. Но между контролируемым экспериментом и наблюдательными данными о зарплатах лежит эндогенность: качество сна не наблюдается ни в одном датасете, а оно одновременно определяет и склонность к депривации, и продуктивность. Человек с хорошим качеством сна высыпается за формально короткое время и остаётся эффективным, поэтому простое сравнение «спал мало» против «спал много» будет систематически смещённым.

Чтобы понять, какие методы с этим справляются, а какие только делают вид, я сгенерировал данные с полностью известным процессом порождения — 10 000 наблюдений, seed 123. В отличие от реальных данных, здесь известен настоящий эффект депривации на каждого работника, поэтому оценки МНК, S/T/X-learner'ов, взвешивания на обратные вероятности, метода двойной устойчивости и двойного машинного обучения можно сравнить не друг с другом, а с правдой.

Весь код — в ноутбуке этого репозитория. Развёрнутый отчёт с ответами на все пункты задания, формулами и промежуточными расчётами — в файле `ДЗ по Машинке.md`. Здесь — сокращённая версия с ключевыми цифрами и выводами.
...
### Наивная оценка и почему она обманывает
...
### Условная независимость не спасает
...
Похожая картина и с индивидуальными эффектами: S-learner из-за линейности своей базовой модели выдаёт один и тот же CATE для всех работников, а X-learner, теоретически заточенный под несбалансированные выборки, здесь не получает преимущества — доли депривированных и нет (41% на 59%) слишком близки, чтобы это сработало. T-learner на этом фоне справляется неожиданно хорошо.

### Инструментальная переменная и её пределы
...
### Насколько чувствительны методы к качеству вспомогательных моделей
...
- Инструмент (ребёнок до 3 лет) хуже удовлетворяет исключающему ограничению, чем внешние по отношению к работнику инструменты вроде времени заката: он способен влиять на производительность и через стресс с финансовой нагрузкой, а не только через сон.
- Депривация задана бинарно (порог 7 часов), хотя реальный недосып скорее дозозависим — бинаризация теряет часть информации о силе воздействия.
...
- `ДЗ по Машинке.md` — развёрнутый отчёт с ответами на все пункты задания, формулами процесса генерации данных и промежуточными расчётами;
